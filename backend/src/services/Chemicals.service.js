...
class ChemicalsService {
  ...
  static async getItemCodesByClass(className) {
    if (className !== "Chemical") {
      return [];
    }

    const itemCodes = await ChemicalsModel.find({}, "item_code").lean();
  ...
  static async UpdateRestockRecordById(restockId, body) {
    ...
    const chemicalRecord = await ChemicalsModel.findById(restockRecord.chemical);
    if (chemicalRecord) {
  ...
  static async GetAllRestocks(filters = {}) {
    ...
    if (filters.chemical) {
      ...
      query.chemical = { $in: matchingChemicals.map((c) => c._id) };
    }
    ...
    const restocks = await ChemicalsRestockModel.find(query)
      .populate("chemical", "item_code item_name") // Populate chemical details
  ...
  static async GetLogs(filters) {
    ...
    if (filters.item_code) {
      chemicalQuery.push({ item_code: { $regex: filters.item_code, $options: "i" } });
    }
    if (filters.item_name) {
      chemicalQuery.push({ item_name: { $regex: filters.item_name, $options: "i" } });
    }

    if (chemicalQuery.length > 0) {
      const chemical = await ChemicalsModel.findOne({ $or: chemicalQuery }).select("_id");

      if (!chemical) {
        console.log("No matching chemical found for given item_code or item_name");
        return []; // Exit early to prevent incorrect logs from being returned
      }

      query.item = chemical._id;
    }
    ...
    // Fetch logs
    let logs = await ChemicalsLogModel.find(query)
  ...
  static async LogIssuedQuantity(item_id, request_model, request, issued_quantity, date_issued, user_email) {
    ...
    if (chemical.current_quantity === 0) {
      ...
      await StockNotificationService.createNotification({
        itemId: chemical._id,
        title: `Out of Stock Alert: ${chemical.item_name}`,
        message: `The chemical ${chemical.item_name} is now out of stock.`,
      ...
      await StockNotificationService.createNotification({
        itemId: chemical._id,
        title: `Low Stock Alert: ${chemical.item_name}`,
        message: `The stock for ${chemical.item_name} is below the minimum level.`,
        send_to: ["admin", "lab-assistant"],
        type: "low_stock",
      });
  ...
  static async UpdateChemicalLogItemById(logId, newIssuedQuantity, user_email, date_issued) {
    ...
    if (chemical.current_quantity === 0) {
      ...
      if (chemical.status === "Out of Stock" || chemical.status === "Low Stock") {
        chemical.status = "In Stock";
        stockRecoveryNotificationSent = true;
        await StockNotificationService.createNotification({
          itemId: chemical._id,
          title: `Stock Recovery: ${chemical.item_name}`,
          message: `The chemical ${chemical.item_name} is now back in stock and no longer out of stock or low stock.`,
          send_to: ["admin", "lab-assistant"],
          type: "stock_recovered",
        });
      }
    ...
    return { msg: "Chemical log updated successfully", stockRecoveryNotificationSent };
  }
