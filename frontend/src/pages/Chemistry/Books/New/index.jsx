...
import BooksCard from './Card.books';
import { useNavigate } from 'react-router-dom';
import { confirmDialog, ConfirmDialog } from 'primereact/confirmdialog';
import { toast } from 'sonner';
import { Button } from 'primereact/button';
import { useDeleteBooksItemMutation } from '../../../../provider/queries/Books.query';
import ReportButton from './ReportButton';

const BooksPage = () => {
    const [visible, setVisible] = useState(false);
    const [searchParams, setSearchParams] = useState({ item_code: '', item_name: '', author:'',location: '', status: '' });
    const [filteredData, setFilteredData] = useState([]); // State to store filtered items
    const [dialogVisible, setDialogVisible] = useState(false);

    const navigate = useNavigate();
    const { isLoading, data, refetch } = useGetAllBooksItemsQuery(searchParams);
    ...
    const [DeleteBooksItem] = useDeleteBooksItemMutation();
                ...
                <div className="p-dialog-footer">
                    <Button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                const { data, error } = await DeleteBooksItem(_id);
                                if (error) {
    ...
    return (
        <div className="w-full flex flex-wrap justify-evenly mt-10">
            <BreadCrumbs PageLink='/Books' PageName='Book' />
            ...
            <div className="mb-3 flex justify-end w-[85%] mx-auto gap-x-6">
                ...
                >
                    Register New Book <GoPlus />
                </button>

                <button
                    onClick={() => navigate('/chemistry/books/restock')}
                ...
                <button
                    onClick={() => navigate('/chemistry/books/logs')}
