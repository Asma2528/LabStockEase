...
import { useUpdateLogMutation } from '../../../../provider/queries/Books.query'; 
...
const UpdateModel = ({ visible, setVisible, log }) => {
    ...
    const initialValues = {
        ...
        date_return: log?.date_return ? format(new Date(log.date_return), 'yyyy-MM-dd') : '',
    };

    const [updateBooksLog] = useUpdateLogMutation(); 
    ...
    const onSubmitHandler = async (values) => {
        try {
            ...
            const response = await updateBooksLog({
                ...
                {({ handleSubmit }) => (
                    <form onSubmit={handleSubmit} className="w-full">
                        ...
                        <div className="mb-3">
                            ...
                            <Field name="date_return" id="date_return" type="date" className="w-full px-5 py-2 rounded-md outline-none border-1 border" />
                            <ErrorMessage name='date_return' component='p' className='text-red-500 text-sm' />
                        </div>
