...
import { useLogIssuedQuantityMutation, useGetAllBooksItemsQuery } from "../../../../provider/queries/Books.query";
...
const Model = ({ visible, setVisible }) => {
  const { data: booksData, isLoading: isBooksLoading } = useGetAllBooksItemsQuery({ query: '' });
  ...
  const handleItemCodeChange = (e, setFieldValue) => {
    const selectedItem = booksData?.items?.find(item => item.item_code === e.target.value);
    if (selectedItem) {
