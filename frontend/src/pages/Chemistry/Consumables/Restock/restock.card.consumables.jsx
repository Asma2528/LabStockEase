...
import ViewRestockItem from './display.restock.consumables'; // Component for viewing restock details
import UpdateRestockModel from './restock.update.consumables';
...
const RestockCard = ({ data, onDelete }) => {
    ...
    if (!data.consumable) {
        return null; // If consumable is null, do not render the card
    }
    ...
    return (
        <>
            <tr className="bg-white border-b hover:bg-gray-50 text-gray-900 whitespace-nowrap">
                {/* Access the item_code and item_name from consumable object */}
                <td className="px-4 py-2 font-medium">{`${data.consumable.item_code} : ${data.consumable.item_name}`}</td>
