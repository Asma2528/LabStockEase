...
import { useUpdateConsumablesItemMutation } from '../../../../provider/queries/Consumables.query';
...
const UpdateModel = ({ visible, setVisible, item }) => {
    ...
    const [updateConsumablesItem, updateConsumablesResponse] = useUpdateConsumablesItemMutation();

    const onSubmitHandler = async (values) => {
        try {
            const response = await updateConsumablesItem({
                data: values,
                id: item._id,
            });
            ...
            <Formik onSubmit={onSubmitHandler} initialValues={initialValues} validationSchema={validationSchema}>
                {({ handleSubmit }) => (
                    <form onSubmit={handleSubmit} className="w-full">
                        ...
                        <div className="mb-3">
                            <label htmlFor="casNo">CAS No <span className="text-red-500 text-sm">*</span></label>
                            <Field name="casNo" id="casNo" type="text" className="w-full px-5 py-2 rounded-md outline-none border-1 border" placeholder="Enter CAS No" />
                            <ErrorMessage name='casNo' component={'p'} className='text-red-500 text-sm' />
                        </div>

                        <div className="mb-3">
                            <label htmlFor="msds">MSDS Link <span className="text-red-500 text-sm">*</span></label>
                            <Field name="msds" id="msds" type="text" className="w-full px-5 py-2 rounded-md outline-none border-1 border" placeholder="Enter MSDS Link" />
                            <ErrorMessage name='msds' component={'p'} className='text-red-500 text-sm' />
                        </div>
                        ...
                        <div className="flex justify-end">
                            <Button type="submit" loading={updateConsumablesResponse.isLoading} className="px-4 rounded-md py-2 bg-blue-900 text-white inline-flex items-center gap-x-2">
                                Update Item
                            </Button>
                        </div>
                    </form>
