...
import { useAddConsumablesItemMutation } from '../../../../provider/queries/Consumables.query';

const Model = ({ visible, setVisible }) => {
    const [addConsumablesItem, { isLoading }] = useAddConsumablesItemMutation();

    const validationSchema = yup.object({
        item_name: yup.string().required("Item name is required"),
        casNo: yup.string().required("CAS No is required"),
        msds: yup.string().required("MSDS link is required"),
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        try {
            const response = await addConsumablesItem(values);
            if (response.error) {
                toast.error(response.error.data.message || 'Failed to add item');
                return;
            }
            toast.success("Consumables Item Added Successfully");
            resetForm();
            setVisible(false);
        } catch (e) {
