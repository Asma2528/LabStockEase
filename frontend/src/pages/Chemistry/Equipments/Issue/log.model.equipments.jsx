...
import { useLogIssuedQuantityMutation, useGetAllEquipmentsItemsQuery } from "../../../../provider/queries/Equipments.query";
...
const Model = ({ visible, setVisible }) => {
  const { data: equipmentsData, isLoading: isEquipmentsLoading } = useGetAllEquipmentsItemsQuery({ query: '' });
  ...
  const handleItemCodeChange = (e, setFieldValue) => {
    const selectedItem = equipmentsData?.items?.find(item => item.item_code === e.target.value);
    if (selectedItem) {
...
/>
            ...
            <Button
              ...
              disabled={isLoading || isEquipmentsLoading}
            >
