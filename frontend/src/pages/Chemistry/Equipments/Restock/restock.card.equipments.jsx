...
import ViewRestockItem from './display.restock.equipments'; // Component for viewing restock details
import UpdateRestockModel from './restock.update.equipments';
...
const RestockCard = ({ data, onDelete }) => {
    ...
    if (!data.equipment) {
        return null; // If equipment is null, do not render the card
    }
    ...
    return (
        <>
            <tr className="bg-white border-b hover:bg-gray-50 text-gray-900 whitespace-nowrap">
                {/* Access the item_code and item_name from equipment object */}
                <td className="px-4 py-2 font-medium">{`${data.equipment.item_code} : ${data.equipment.item_name}`}</td>
