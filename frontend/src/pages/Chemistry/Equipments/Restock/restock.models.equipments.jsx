...
import { useAddRestockItemMutation, useGetAllEquipmentsItemsQuery } from '../../../../provider/queries/Equipments.query';  
...
const RestockModel = ({ visible, setVisible }) => {
    ...
    // Fetch all equipments initially
    const { data: equipmentsData } = useGetAllEquipmentsItemsQuery({ query: '' });
    const { data: inwardData } = useGetAllInwardsQuery(); 
    console.log(inwardData);
    // Update the equipmentOptions when data is available
    useEffect(() => {
      
        if (equipmentsData?.items) { // Access the items array
            const options = equipmentsData.items.map(equipment => ({
                value: equipment._id,
                label: `${equipment.item_code} - ${equipment.item_name}`,
            }));
    ...
    // Handle selection of a equipment
                    ...
                    <form onSubmit={handleSubmit} className="w-full">
                        ...
                        <div className="mb-3">
                            ...
                            <Select
                                ...
                                value={selectedEquipment} // Ensure Select dropdown value is in sync
                            />
                            <ErrorMessage name='equipment' component={'p'} className='text-red-500 text-sm' />
                        </div>
