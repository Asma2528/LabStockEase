...
import { useUpdateEquipmentsItemMutation } from '../../../../provider/queries/Equipments.query';
...
const UpdateModel = ({ visible, setVisible, item }) => {
    ...
    const [updateEquipmentsItem, updateEquipmentsResponse] = useUpdateEquipmentsItemMutation();

    const onSubmitHandler = async (values) => {
        try {
            const response = await updateEquipmentsItem({
                data: values,
                id: item._id,
            });
            ...
            <Formik onSubmit={onSubmitHandler} initialValues={initialValues} validationSchema={validationSchema}>
                {({ handleSubmit }) => (
                    <form onSubmit={handleSubmit} className="w-full">
                        ...
                        <div className="mb-3">
                            <label htmlFor="serial_number">Serial Number <span className="text-red-500 text-sm">*</span></label>
                            <Field name="serial_number" id="serial_number" type="text" className="w-full px-5 py-2 rounded-md outline-none border-1 border" placeholder="Enter Serial Number" />
                            <ErrorMessage name='serial_number' component={'p'} className='text-red-500 text-sm' />
                        </div>

                        <div className="mb-3">
                            <label htmlFor="model_number">Model Number <span className="text-red-500 text-sm">*</span></label>
                            <Field name="model_number" id="model_number" type="text" className="w-full px-5 py-2 rounded-md outline-none border-1 border" placeholder="Enter Model Number" />
                            <ErrorMessage name='model_number' component={'p'} className='text-red-500 text-sm' />
                        </div>

                        <div className="mb-3">
                            <label htmlFor="manual">Manual Link <span className="text-red-500 text-sm">*</span></label>
                            <Field name="manual" id="manual" type="text" className="w-full px-5 py-2 rounded-md outline-none border-1 border" placeholder="Enter Manual Link" />
                            <ErrorMessage name='manual' component={'p'} className='text-red-500 text-sm' />
                        </div>
                        ...
                        <div className="flex justify-end">
                            <Button type="submit" loading={updateEquipmentsResponse.isLoading} className="px-4 rounded-md py-2 bg-blue-900 text-white inline-flex items-center gap-x-2">
                                Update Item
                            </Button>
                        </div>
                    </form>
