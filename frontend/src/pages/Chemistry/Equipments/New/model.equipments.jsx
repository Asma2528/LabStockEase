...
import { useAddEquipmentsItemMutation } from '../../../../provider/queries/Equipments.query';

const Model = ({ visible, setVisible }) => {
    const [addEquipmentsItem, { isLoading }] = useAddEquipmentsItemMutation();

    const validationSchema = yup.object({
        item_name: yup.string().required("Item name is required"),
        serial_number: yup.string().required("Serial Number is required"),
        model_number: yup.string().required("Serial Number is required"),
        manual: yup.string().required("Manual link is required"),
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        try {
            const response = await addEquipmentsItem(values);
            if (response.error) {
                toast.error(response.error.data.message || 'Failed to add item');
                return;
            }
            toast.success("Equipments Item Added Successfully");
            resetForm();
            setVisible(false);
        } catch (e) {
