...
const InventoryTable = ({ title, items, showExpiry = false, formatDate }) => (
          ...
          {items.length > 0 ? (
            items.map((item) => (
              <tr key={item._id}>
                ...
                <td className="px-4 py-2">{item.total_quantity || 0}</td>
                <td className="px-4 py-2">{item.current_quantity || 0}</td>
                <td className="px-4 py-2">{item.min_stock_level || "N/A"}</td>
                {showExpiry && <td className="px-4 py-2">{formatDate(item.expiration_date)}</td>}
              </tr>
            ))
          ) : (
            <tr>
