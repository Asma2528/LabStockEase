...
const Model = ({ visible, setVisible }) => {
  const { data: chemicalsData, isLoading: isChemicalsLoading } = useGetAllChemicalsItemsQuery({ query: '' });
  ...
  const handleItemCodeChange = (e, setFieldValue) => {
    const selectedItem = chemicalsData?.items?.find(item => item.item_code === e.target.value);
    if (selectedItem) {
