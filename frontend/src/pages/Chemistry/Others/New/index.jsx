...
import Model from './model.others';
import { GoPlus } from "react-icons/go";
import { useGetAllOthersItemsQuery } from '../../../../provider/queries/Others.query';
import Loader from '../../../../components/Loader';
import OthersCard from './Card.others';
import { useNavigate } from 'react-router-dom';
import { confirmDialog, ConfirmDialog } from 'primereact/confirmdialog';
import { toast } from 'sonner';
import { Button } from 'primereact/button';
import { useDeleteOthersItemMutation } from '../../../../provider/queries/Others.query';
import ReportButton from './ReportButton';

const OthersPage = () => {
    const [visible, setVisible] = useState(false);
    const [searchParams, setSearchParams] = useState({ item_code: '', item_name: '', category:'',company: '', status: '' });
    const [filteredData, setFilteredData] = useState([]); // State to store filtered items
    const [dialogVisible, setDialogVisible] = useState(false);

    const navigate = useNavigate();
    const { isLoading, data, refetch } = useGetAllOthersItemsQuery(searchParams);
    ...
    const [DeleteOthersItem] = useDeleteOthersItemMutation();
                ...
                <div className="p-dialog-footer">
                    <Button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                const { data, error } = await DeleteOthersItem(_id);
                                if (error) {
    ...
    return (
        <div className="w-full flex flex-wrap justify-evenly mt-10">
            <BreadCrumbs PageLink='/Others' PageName='Other' />
            ...
            <div className="mb-3 flex justify-end w-[85%] mx-auto gap-x-6">
                ...
                <button
                    onClick={() => navigate('/chemistry/others/restock')}
                ...
                <button
                    onClick={() => navigate('/chemistry/others/logs')}
