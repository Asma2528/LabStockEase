...
import { useAddRestockItemMutation, useGetAllOthersItemsQuery } from '../../../../provider/queries/Others.query';  
...
const RestockModel = ({ visible, setVisible }) => {
    ...
    // Fetch all others initially
    const { data: othersData } = useGetAllOthersItemsQuery({ query: '' });
    const { data: inwardData } = useGetAllInwardsQuery(); 
    console.log(inwardData);
    // Update the otherOptions when data is available
    useEffect(() => {
      
        if (othersData?.items) { // Access the items array
            const options = othersData.items.map(other => ({
                value: other._id,
                label: `${other.item_code} - ${other.item_name}`,
            }));
    ...
    // Handle selection of a other
    const handleOtherSelect = (selectedOption) => {
        setSelectedOther(selectedOption);
    };
    ...
    const handleSearchInputChange = (inputValue) => {
        const filtered = otherOptions.filter(option =>
            option.label.toLowerCase().includes(inputValue.toLowerCase())
        );
                    ...
                    <form onSubmit={handleSubmit} className="w-full">
                        ...
                        <div className="mb-3">
                            ...
                            <Select
                                ...
                                value={selectedOther} // Ensure Select dropdown value is in sync
                            />
                            <ErrorMessage name='other' component={'p'} className='text-red-500 text-sm' />
                        </div>
