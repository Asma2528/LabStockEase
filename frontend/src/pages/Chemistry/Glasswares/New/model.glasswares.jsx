...
import { useAddGlasswaresItemMutation } from '../../../../provider/queries/Glasswares.query';

const Model = ({ visible, setVisible }) => {
    const [addGlasswaresItem, { isLoading }] = useAddGlasswaresItemMutation();
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        try {
            const response = await addGlasswaresItem(values);
            if (response.error) {
                toast.error(response.error.data.message || 'Failed to add item');
                return;
            }
            toast.success("Glasswares Item Added Successfully");
            resetForm();
            setVisible(false);
        } catch (e) {
