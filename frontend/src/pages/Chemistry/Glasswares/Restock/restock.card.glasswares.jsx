...
import ViewRestockItem from './display.restock.glasswares'; // Component for viewing restock details
import UpdateRestockModel from './restock.update.glasswares';
...
const RestockCard = ({ data, onDelete }) => {
    ...
    if (!data.glassware) {
        return null; // If glassware is null, do not render the card
    }
    ...
    return (
        <>
            <tr className="bg-white border-b hover:bg-gray-50 text-gray-900 whitespace-nowrap">
                {/* Access the item_code and item_name from glassware object */}
                <td className="px-4 py-2 font-medium">{`${data.glassware.item_code} : ${data.glassware.item_name}`}</td>
