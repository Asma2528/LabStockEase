...
import { useAddRestockItemMutation, useGetAllGlasswaresItemsQuery } from '../../../../provider/queries/Glasswares.query';  
...
const RestockModel = ({ visible, setVisible }) => {
    ...
    const [glasswareOptions, setGlasswareOptions] = useState([]);
    ...
    // Fetch all glasswares initially
    const { data: glasswaresData } = useGetAllGlasswaresItemsQuery({ query: '' });
    const { data: inwardData } = useGetAllInwardsQuery(); 
    console.log(inwardData);
    // Update the glasswareOptions when data is available
    useEffect(() => {
      
        if (glasswaresData?.items) { // Access the items array
            const options = glasswaresData.items.map(glassware => ({
                value: glassware._id,
                label: `${glassware.item_code} - ${glassware.item_name}`,
            }));
            setGlasswareOptions(options);
    ...
    const validationSchema = yup.object({
        glassware: yup.string().required("Glassware is required"),
    ...
    const initialValues = {
        glassware: selectedGlassware?.value || '', 
    ...
    // Handle selection of a glassware
    const handleGlasswareSelect = (selectedOption) => {
        setSelectedGlassware(selectedOption);
    };
    ...
    const handleSearchInputChange = (inputValue) => {
        const filtered = glasswareOptions.filter(option =>
            option.label.toLowerCase().includes(inputValue.toLowerCase())
        );
                    ...
                    <form onSubmit={handleSubmit} className="w-full">
                        ...
                        <div className="mb-3">
                            ...
                            <Select
                                ...
                                onChange={handleGlasswareSelect}
                                placeholder="Search by item code or name"
                                value={selectedGlassware} // Ensure Select dropdown value is in sync
                            />
                            <ErrorMessage name='glassware' component={'p'} className='text-red-500 text-sm' />
                        </div>
