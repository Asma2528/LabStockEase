...
import { useLogIssuedQuantityMutation, useGetAllGlasswaresItemsQuery } from "../../../../provider/queries/Glasswares.query";
...
const Model = ({ visible, setVisible }) => {
  const { data: glasswaresData, isLoading: isGlasswaresLoading } = useGetAllGlasswaresItemsQuery({ query: '' });
  ...
  const handleItemCodeChange = (e, setFieldValue) => {
    const selectedItem = glasswaresData?.items?.find(item => item.item_code === e.target.value);
    if (selectedItem) {
...
/>
            ...
            <Button
              ...
              disabled={isLoading || isGlasswaresLoading}
            >
