...
import { useCreateOrderRequestMutation } from '../../provider/queries/Order.Request.query';
...
const Modal = ({ visible, setVisible }) => {
  const [createOrderRequest, { isLoading }] = useCreateOrderRequestMutation();
 ...
 categoryType: values.categoryType,
        ...
        items: values.items.map((item) => ({
          ...item,
          quantity_returned: 0, // Initialize quantity_returned
        })),
      ...
      const response = await createOrderRequest(orderRequestData);

      if (response.error) {
        toast.error(response.error.data.message || 'Failed to create order request');
        return;
      }
         ...
         />
            ...
            <tr>
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Creating...' : 'Create Order Request'}
              </Button>
            </form>
          );
        }}
