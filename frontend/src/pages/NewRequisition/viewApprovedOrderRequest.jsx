...
import { useGetApprovedAndOrderedRequestsQuery,useChangeStatusToOrderedMutation } from '../../provider/queries/Order.Request.query';
import Loader from '../../components/Loader';
import { toast } from 'sonner';
import { Button } from 'primereact/button';
import { FaRegEdit } from 'react-icons/fa';

const ViewApprovedOrderRequestPage = () => {
  ...
  const [orderedOrderRequest] = useChangeStatusToOrderedMutation();
...
console.log(data);
  ...
  const handleDialogConfirm = async () => {
    try {
      ...
      await orderedOrderRequest({
        id: selectedOrderRequest._id,
        status: "Ordered",
      }).unwrap();
      
      toast.success(`Order Requests ${statusValue}`);
      setIsDialogVisible(false);
      refetch();
    } catch (error) {
      console.error('Ordered error:', error);
      const errorMsg = error?.data?.msg || error?.data?.message || 'Failed to update order requests';
  ...
  return (
    <div className="w-full flex flex-wrap justify-evenly mt-10">
      <BreadCrumbs PageLink='/view-approved-order-requests' PageName='Approve Order Requests' />
  ...
  <div className="fixed inset-0 z-50 flex justify-center items-center bg-gray-500 bg-opacity-50">
    ...
    <h2 className="text-xl font-semibold mb-4">Change Status to Ordered: Order Request</h2>
      {/* Close Button */}
      <button
            ...
            />
                ...
                <div className="mb-3">
                    ...
                    <input 
                        ...
                        type="text" 
                        className="w-full px-5 py-2 rounded-md outline-none border-1 border" 
                        value={formatDate(selectedOrderRequest.orderedd_at)} 
                        disabled 
                    />
                </div>
  ...
  />
          ...
          <div className="mb-3">
            ...
            <textarea
              ...
              rows="3"
              value={selectedOrderRequest.remark}
            />
          </div>
...
<Button
  ...
  icon="pi pi-check"
  className={`p-button-danger py-2 px-4 rounded-md ${selectedOrderRequest?.status === 'Approved' ? 'bg-green-400 text-white' : 'bg-gray-700 cursor-not-allowed text-white'}`}
  onClick={handleDialogConfirm}
  disabled={selectedOrderRequest?.status === 'Ordered' }
/>
...
export default ViewApprovedOrderRequestPage;
