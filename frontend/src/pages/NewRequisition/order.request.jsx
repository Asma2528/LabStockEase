...
import Model from "./model.order.request"; // Create Requisition Modal
import { GoPlus } from "react-icons/go";
import {useGetUserOrderRequestsQuery,useDeleteOrderRequestMutation} from "../../provider/queries/Order.Request.query";
import Loader from "../../components/Loader";
import OrderRequestCard from "./card.order.request";
...
const OrderRequestPage = () => {
  ...
  const { isLoading, data, refetch } = useGetUserOrderRequestsQuery(searchParams);
  ...
  const onSearchChange = (e) => {
   ...
   const handleDelete = (id) => {
    ...
    confirmDialog({
      message: 'Are you sure you want to delete this order request?',
      ...
      footer: (
        <div className="p-dialog-footer flex justify-end gap-2">
          <Button
            ...
            onClick={async () => {
              try {
                const { data, error } = await deleteOrderRequest(id);
                if (error) {
  ...
  return (
    <div className="w-full flex flex-col items-center mt-10">
      <BreadCrumbs PageLink="/Order-Request" PageName="Order Request" />
  ...
  {Array.isArray(filteredData) && filteredData.length > 0 ? (
    filteredData.map((orderRequest) => (
      <OrderRequestCard
        ...
        data={orderRequest}
        onDelete={() => handleDelete(orderRequest._id)}
      />
