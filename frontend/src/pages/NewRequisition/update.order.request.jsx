...
import { useUpdateOrderRequestMutation } from '../../provider/queries/Order.Request.query';
...
const UpdateModal = ({ visible, setVisible, orderRequest }) => {
  const [updateOrderRequest, { isLoading }] = useUpdateOrderRequestMutation();
  ...
  const initialValues = {
    categoryType: orderRequest?.categoryType || '',
        category: orderRequest?.category || '',
    date_of_requirement: orderRequest.date_of_requirement || '',
 ...
 console.log("Submitting Order Request with values:", values);
      ...
      const orderRequestData = {
        ...
        status: orderRequest.status, // Status should not be updated
        requested_by: orderRequest.requested_by._id, // Cannot modify requested_by
        approved_by: orderRequest.approved_by?._id || null, // Cannot modify approved_by
        ordered_by: orderRequest.ordered_by?._id || null, // Cannot modify ordered_by
        approved_at: orderRequest.approved_at, // Cannot modify approved_at
        ordered_at: orderRequest.ordered_at, // Cannot modify ordered_at
      };




      const response = await updateOrderRequest({ id: orderRequest._id, updateData:orderRequestData });

      if (response.error) {
        toast.error(response.error.data.message || 'Failed to update Order Request');
        return;
      }
          ...
          return (
            <form onSubmit={handleSubmit} className="w-full">
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Updating...' : 'Update Order Request'}
              </Button>
            </form>
          );
        }}
