...
import ViewItem from './display.order.request';
import UpdateModel from './update.order.request';

const OrderRequestCard = ({ data ,onDelete}) => {
  ...
  return (
    <>
      ...
      <td className="px-4 py-2 font-medium">{data.orderRequest_code || "N/A"}</td>
      ...
      {/* Modals */}
      <ViewItem visible={visible} setVisible={setVisible} orderRequest={data} />
      <UpdateModel visible={updateVisible} setVisible={setUpdateVisible} orderRequest={data} />
    </>
  );
};
...
OrderRequestCard.propTypes = {
  data: PropTypes.shape({
    ...
    orderRequest_code: PropTypes.string.isRequired,
