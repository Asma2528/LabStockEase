...
import { useUpdateGeneralMutation } from '../../provider/queries/General.query';

const UpdateGeneralModel = ({ visible, setVisible, general }) => {
    const [updateGeneral, { isLoading }] = useUpdateGeneralMutation();

    // Validation schema for general fields
    ...
    const initialValues = {
        ...
        generalDate: general?.generalDate || '',
        sanctionDate: general?.sanctionDate || '',
        generalPeriod: general?.generalPeriod || '',
        fundingAgency: general?.fundingAgency || '',
        generalCost: general?.generalCost || '',
        fundStatus: general?.fundStatus || '',
        generalInCharge: general?.generalInCharge || '',
        generalStatus: general?.generalStatus || '',
    };

    // Form submission handler
    const onSubmitHandler = async (values) => {
        try {
            const response = await updateGeneral({ id: general._id, ...values });

            // Handle error response
            if (response.error) {
                toast.error(response.error.data.message || 'Failed to update general');
                return;
            }
            ...
            >
              ...
              <div className="mb-3">
                        ...
                        <Button type="submit" className="w-full bg-blue-900 text-center text-white py-3 flex items-center justify-center" disabled={isLoading}>
                            {isLoading ? 'Updating...' : 'Update General'}
                        </Button>
                    </form>
                )}
            </Formik>
...
UpdateGeneralModel.propTypes = {
    visible: PropTypes.bool.isRequired,
    setVisible: PropTypes.func.isRequired,
    general: PropTypes.object.isRequired,
};

export default UpdateGeneralModel;
