...
import { useUpdateOrderMutation } from '../../provider/queries/Orders.query';
...
const UpdateOrderModel = ({ visible, setVisible, order }) => {
console.log(order);
    ...
    useEffect(() => {
    }, [projects, practicals, other, general, vendors]);
    ...
    const initialValues = {
        ...
        quotationDate: order?.quotationDate ? new Date(order.quotationDate).toISOString().split('T')[0] : '',
    ...
    const { data: chemicalData } = useGetAllChemicalsItemsQuery({});
    ...
    const getItemOptions = (selectedClass) => {
        switch (selectedClass) {
            case 'Chemicals':
                return chemicalData?.items?.map(item => ({ label: `${item.item_code} - ${item.item_name}`, value: item.item_code })) || [];
            case 'Equipments':
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        try {
            ...
            const totalCost = values.items.reduce((sum, item) => sum + (Number(item.cost) || 0), 0);
            const totalGST = Number(values.totalGST) || 0;

            // Correct Calculation
            const grandTotal = totalCost + totalGST;

            // Store properly formatted values
            values.totalCost = totalCost.toFixed(2);
            values.totalGST = totalGST.toFixed(2);
            values.grandTotal = grandTotal.toFixed(2);

            const response = await updateOrder({ id: order._id, ...values });

            if (response.error) {
                toast.error(response.error.data.message || 'Failed to update order');
                return;
            }
    ...
    return (
        <Dialog header="Update Order" visible={visible} className="w-full md:w-[98%] lg:w-[95%]" onHide={() => setVisible(false)}>
                ...
                {({ handleSubmit, values, setFieldValue }) => {
                    ...
                    return (
                        <form onSubmit={handleSubmit} className="w-full">
                            {/* Basic Order Info */}
                            {['quotationRefNo', 'quotationDate'].map((field, index) => (
                            ...
                            <div className="mb-3">
                                <label>Vendor</label>
                                <Dropdown
                                    value={values.vendor}  // Set the vendor ID as value
                                    options={vendors?.vendors?.map(vendor => ({
                                        label: vendor.name,  // Display the vendor name in the dropdown
                                        value: vendor._id  // Use _id for the value
                                    })) || []}
                                    onChange={(e) => setFieldValue('vendor', e.value)}  // Update the value (vendor ID)
                            ...
                            <div className="mb-3">
                                <label>Status</label>
                                <Field as="select" name="status" className="w-full px-5 py-2 rounded-md border" disabled={['Pending', 'Rejected', 'Received'].includes(order.status)}>
                                    {/* Show the current status as the default option */}
                                    <option value={order.status}>{order.status}</option>

                                    {/* Show options based on current status */}
                                    {order.status === 'Approved' && (
                                        <>
                                            <option value="Placed" selected={values.status === 'Placed'}>Placed</option>
                                            <option value="Received" selected={values.status === 'Received'}>Received</option>
                                        </>
                                    )}

                                    {order.status === 'Placed' && (
                                        // Allow only "Received" status if the current status is "Placed"
                                        <option value="Received" selected={values.status === 'Received'}>Received</option>
                                    )}
                                </Field>
