...
import { useAddOtherMutation } from '../../provider/queries/Other.query';


const OtherModel = ({ visible, setVisible }) => {
    const [addOther, { isLoading }] = useAddOtherMutation();
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        try {
            const response = await addOther(values);

            if (response.error) {
                toast.error(response.error.data.message || 'Failed to add other');
                return;
            }

            toast.success("Other Added Successfully");
            resetForm();
            setVisible(false);
        } catch (e) {
            console.error(e);
